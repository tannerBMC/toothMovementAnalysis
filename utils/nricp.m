function [ vertsTransformed, X, rigidM, meanAbsErr ] = nricp( Source, Target, Options )
% nricp performs an adaptive stiffness variant of non rigid ICP.
%
% [ vertsTransformed, X, rigidM, meanAbsErr ] = nricp( Source, Target, Options )
%
...
%
% 2022/12/08 CT added option noFinalSnapping
...
if ~isfield(Options, 'rigidM')
    Options.rigidM = [eye(3); [0 0 0]];
end
...
if Options.rigidInit == 1
    ...
    rigidM = [R'; t'];
    X = repmat(rigidM, nVertsSource, 1);
...
meanAbsErr=100;  % dummy value for output
...
for i = 1:nAlpha
    ...
    noItr = 1;
    normDiff = norm(X - oldX);
    threshold = Options.epsilon;
    while normDiff >= threshold
        
        if sum(sum(isnan(X)))>0
            keyboard
        end
        
        disp([num2str(alpha) ' ' num2str(noItr) ' transfDifference T-oldT: ' num2str(normDiff,'%.2f') ' absErr:' num2str(meanAbsErr,'%.4f')])
        ...
        % mean absolute error before optimization
        absErrV=W*abs(U-vertsTransformed);
        meanAbsErr=mean(absErrV(:));
        ...
        X = (A' * A) \ (A' * B);
        if sum(sum(isnan(X)))>0
            % keep old result, no further optimization at this level
            X=oldX;
            %keyboard
        end
        normDiff = norm(X - oldX);
...
absErrV=W*abs(U-vertsTransformed);
meanAbsErr=mean(absErrV(:));
disp([num2str(alpha) ' final transfDifference T-oldT: ' num2str(normDiff,'%.2f') ' absErr:' num2str(meanAbsErr,'%.4f')])
...
if Options.useNormals == 1
    ...
    vertsTransformed = projectNormals(vertsTransformed, Target, ...
                                       normalsTransformed);
elseif Options.noFinalSnapping == 1
    disp('keeping transformed vertices instead of snapping template to nearest point')
else
